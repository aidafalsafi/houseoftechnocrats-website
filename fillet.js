...
(function () {
  ...
  var shellSelector = [
    ...
    ".radar-category-card",
    ".radar-live-detail",
    ".radar-landing-report",
    ".radar-news-card",
    ".radar-participation-section",
    ".record-side-card",
    ".record-hero",
    ".record-question-panel",
    ".record-pattern-card",
    ".record-evidence-card",
    ".record-related-card",
    ".person-profile-photo",
    ".profile-feature-hero",
    ".profile-story-section",
    ".profile-section-block",
    ".profile-experience-section",
    ".profile-monograph-statement",
    ".profile-continuity-card",
    ".profile-domain-card",
    ".profile-info-card",
    ".profile-status-bar",
    ".pattern-cta",
  ...
  function path(width, height, corners, cut, radius, fillet, radii) {
  ...
  function cornerRadii(element, inset) {
    return ["tl", "tr", "br", "bl"].reduce(function (result, corner) {
      var value = element.getAttribute("data-radius-" + corner);
      if (value != null) result[corner] = Math.max(+value - (inset || 0), 0);
      return result;
    }, {});
  }

  function shellTone(element) {
    if (element.classList.contains("profile-monograph-statement")) return "paper";
  ...
  function ensureShell(element) {
    ...
    if (element.classList.contains("anchor-tabs")) {
      shell.classList.add("dbr-anchor-tabs-shell");
    }
    if (element.matches(".house-cta, .participate-strip, .inner-hero, .inner-section, .profile-feature-hero, .profile-story-section, .profile-section-block, .profile-experience-section")) {
    ...
    ["tl", "tr", "br", "bl"].forEach(function (corner) {
      var value = element.getAttribute("data-radius-" + corner);
      if (value != null) shell.setAttribute("data-radius-" + corner, value);
    });
    ...
    ["tl", "tr", "br", "bl"].forEach(function (corner) {
      element.removeAttribute("data-radius-" + corner);
    });
    return shell;
  }
  ...
  function applyDirect(element) {
    ...
    element.style.clipPath = corners.length
      ? 'path("' + path(bounds.width, bounds.height, corners, cut, radius, fillet, cornerRadii(element, 0)) + '")'
      : "none";
    element.style.borderRadius = corners.length ? "0" : radius + "px";
  }
  ...
  function applyShell(shell) {
    ...
    if (corners.length) {
      shell.style.clipPath = 'path("' + path(outerBounds.width, outerBounds.height, corners, cut, radius, fillet, cornerRadii(shell, 0)) + '")';
      inner.style.clipPath = 'path("' + path(innerBounds.width, innerBounds.height, corners, Math.max(cut - 1, 0), Math.max(radius - 1, 0), fillet, cornerRadii(shell, 1)) + '")';
  ...
  document.addEventListener("load", function (event) {
    if (event.target && event.target.tagName === "IMG") apply();
  }, true);
